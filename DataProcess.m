...
MaxOutput = prctile(abs(data(:,16:22)),99.9,1);
MinOutput = -MaxOutput;
MaxTheta = [3.14,3.14,3.14,3.14,3.14,3.14,3.14];
MinTheta = -MaxTheta;
MaxThetaDot = [0.5,0.5,0.5,0.5,0.5,0.5,0.5];
...
csvwrite('TrainingDataNotMixed.csv', TrainingProcessed);
...
%        end
       ...
       for joint = 1:num_output
           Processed(DataIdx, num_joint*num_feature*num_sequence + joint) = 2*(RawData(k,1+num_joint*num_feature+joint) - MinOutput(joint))/(MaxOutput(joint) - MinOutput(joint))-1;
       end
